...
public final class GeoExcelTemplateWriter {
    ...
    public static void writeDaily(OutputStream out) throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            sheet.addMergedRegion(new CellRangeAddress(0, 0, start, start + 11));
            for (int m = 0; m < metrics.length; m++) {
                int col = start + m * 2;
                text(row1, col, metrics[m], styles.header);
                sheet.addMergedRegion(new CellRangeAddress(1, 1, col, col + 1));
                for (int p = 0; p < platforms.length; p++) {
                    // 第3行前两格会被导入器当成平台名，不能加「必填」后缀
                    text(row2, col + p, platforms[p], m == 0 ? styles.required : styles.header);
                }
            }
            ...
            text(sample, 4, "是", styles.sample);
            text(sample, 5, "否", styles.sample);
            text(sample, 6, "3", styles.sample);
            text(sample, 7, "", styles.sample);
            text(sample, 8, "出现且推荐", styles.sample);
            text(sample, 9, "未出现", styles.sample);
            text(sample, 10, "", styles.sample);
            text(sample, 11, "https://chat.deepseek.com/share/template-demo", styles.sample);
            text(sample, 12, "", styles.sample);
            text(sample, 13, "", styles.sample);
            text(sample, 14, "竞品A", styles.sample);
            text(sample, 15, "", styles.sample);
            ...
            writeInstructions(wb, styles, "填写说明", List.of(
                    List.of("巡查日期", "是", "第1行、每个日期块的第1列，格式 yyyy-MM-dd。一个日期占后面连续 12 列（2个平台 × 6个指标）", "2026-09-01"),
                    List.of("平台名", "是", "第3行、日期块前两格，黄底红字。名称必须与系统 AI 平台一致，不要加「必填」等后缀。样例为豆包、DS", "豆包 / DS"),
                    List.of("话题", "是", "C列。同一话题的后续行可留空，沿用上一行", "新疆特产"),
    ...
    public static void writePlacement(OutputStream out) throws IOException {
        try (Workbook wb = new XSSFWorkbook()) {
            Styles styles = new Styles(wb);
            Sheet sheet = wb.createSheet("内容投放导入");
            String[] headers = {
                    "序号（必填）", "发布人", "负责人", "话题（必填）", "目标问题（必填）", "标题",
                    "链接或状态", "发布平台", "发布时间", "提问问题", "", "", "", "备注"
            };
            Row row0 = sheet.createRow(0);
            for (int i = 0; i < headers.length; i++) {
                boolean required = headers[i].contains("必填");
                text(row0, i, headers[i], required ? styles.required : styles.header);
            }
            Row row1 = sheet.createRow(1);
            text(row1, 9, "提问问题", styles.header);
            text(row1, 10, "豆包", styles.header);
            text(row1, 11, "DS", styles.header);
            text(row1, 12, "元宝", styles.header);
            ...
            String[] sampleCells = {
                    ...
                    "2026-09-01",
                    SAMPLE_MARK + "新疆适合寄内地的礼品有哪些",
                    "https://www.doubao.com/thread/template-demo",
                    "",
                    "",
                    SAMPLE_MARK + "请删除本行及下一行后再填写正式数据"
            };
            for (int i = 0; i < sampleCells.length; i++) {
                text(sample, i, sampleCells[i], styles.sample);
            }
            Row follow = sheet.createRow(3);
            text(follow, 6, "未投放", styles.sample);
            text(follow, 7, "知乎", styles.sample);
            text(follow, 13, SAMPLE_MARK + "同一序号的后续平台行，序号留空", styles.sample);

            sheet.createFreezePane(0, 2);
            int[] widths = {16, 14, 14, 16, 42, 36, 36, 14, 16, 42, 42, 28, 28, 42};
            ...
            writeInstructions(wb, styles, "填写说明", List.of(
                    ...
                    List.of("话题", "是", "按名称匹配已有话题，没有则新建", "新疆特产"),
                    ...
                    List.of("提问问题", "有引用时必填", "和第 11–13 列的链接同时有值，才会记下 AI 引用", "新疆适合寄内地的礼品有哪些"),
                    List.of("豆包 / DS / 元宝", "否", "对应平台的引用链接，只填有引用的列即可", "https://www.doubao.com/thread/xxx"),
                    List.of("备注", "否", "写入内容投放主表备注", ""),
